...
class Redd_Parser:
                ...
                for appliance in self.appliance_names:
                    if app_index_dict[appliance][0] == -1:
                        ...
                        temp_data = pd.read_csv(
                            house_folder / f"channel_{app_index_dict[appliance][0]}.dat", sep=" ", header=None
                        )
    ...
    def get_train_datasets(self):
        ...
        train = NILMDataset(
            self.x[val_end:], self.y[val_end:], self.status[val_end:], self.window_size, self.window_stride
        )

        val = NILMDataset(
            self.x[:val_end], self.y[:val_end], self.status[:val_end], self.window_size, self.window_size
