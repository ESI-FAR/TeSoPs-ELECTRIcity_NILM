...
class PChaingerParser:
    ...
    def load_data(self, location):
        ...
        )  # force values to be between 0 and cutoff
        house_data = house_data.fillna(0)

        if self.drop_last:
            length_of_incomplete_chunk = len(house_data.index) % self.window_size
            return pd.to_datetime(house_data.index[:-length_of_incomplete_chunk]), house_data.reset_index(
                drop=True
            ).values[:-length_of_incomplete_chunk, 0]
        else:
