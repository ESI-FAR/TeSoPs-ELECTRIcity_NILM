...
from .metrics import acc_precision_recall_f1_score
...
class Trainer:
    def __init__(self, args, ds_parser, model):
        ...
        # per epoch
        self.train_metrics_dict = {"mae": [], "mre": [], "acc": [], "precision": [], "recall": [], "f1": []}
        # per validate() run
        self.val_metrics_dict = {"mae": [], "mre": [], "acc": [], "precision": [], "recall": [], "f1": []}
        # test set
        self.test_metrics_dict = {"mae": [], "mre": [], "acc": [], "precision": [], "recall": [], "f1": []}
        ...
        with torch.no_grad():
            ...
            for _, batch in enumerate(tqdm_dataloader):
                ...
                tqdm_dataloader.set_description(
                    "Validation, rel_err {:.2f}, acc {:.2f}, f1 {:.2f}".format(mre_mean, acc_mean, f1_mean)
                )
    ...
    def test(self, test_loader, map_location="cpu"):
        ...
        with torch.no_grad():
            ...
            for _, batch in enumerate(tqdm_dataloader):
                ...
                tqdm_dataloader.set_description(
                    "Test, rel_err {:.2f}, acc {:.2f}, f1 {:.2f}".format(mre_mean, acc_mean, f1_mean)
                )
